...
This module provides utilities to inspect, validate, and diagnose
data coming from devices and identify performance bottlenecks.
"""
...
def diagnose_io_database(io_database: IODatabase) -> Dict[str, Any]:
    ...
    Args:
        io_database: IODatabase instance to diagnose
    ...
    for channel_path in all_channel_paths:
        ...
        # Sample first few data points for validation
        if channel_data.data_points:
            sample_points = []
            for i, point in enumerate(list(channel_data.data_points)[:5]):
                sample_points.append({
                    'value': str(point.value)[:50],  # Truncate long values
...
def diagnose_virtual_database_build(
    ...
    Args:
        virtual_db: VirtualDatabase instance to diagnose
        max_snapshot_size: Maximum snapshot size to check against
...
def validate_data_point(point: DataPoint, channel_path: str) -> List[str]:
    ...
    Args:
        point: DataPoint to validate
        channel_path: Channel path for context
    ...
    if abs(point.elapsed_time) > 86400 * 365:  # More than a year
        issues.append(
...
def print_diagnosis(diagnosis: Dict[str, Any]) -> None:
    ...
    Args:
        diagnosis: Diagnosis dictionary from diagnose functions
    """
    print("\n" + "="*60)
    ...
    if diagnosis.get('channels'):
        ...
        for channel_path, info in list(diagnosis['channels'].items())[:10]:  # Limit to first 10
